...
def test_compute_rolling_metrics_pandas(mock_market_data):
    df_metrics = compute_rolling_metrics_pandas(mock_market_data['pandas'], "AAPL", ['price'], window=20)
...
def test_compute_rolling_metrics_polars(mock_market_data):
    df_metrics = compute_rolling_metrics_polars(mock_market_data['polars'], "AAPL", ['price'], window=20)
...
def test_pandas_polars_equivalence(mock_market_data):
    df_metrics_pd = compute_rolling_metrics_pandas(mock_market_data['pandas'], "AAPL", ['price'], window=20)
    df_metrics_pl = compute_rolling_metrics_polars(mock_market_data['polars'], "AAPL", ['price'], window=20)

    # ensure results are the same for any random index
    test_index = np.random.randint(1, 101)
