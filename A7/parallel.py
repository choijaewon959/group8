...
if __name__ == "__main__":
    ...
    multiprocessing_results, total_time, avg_cpu, mem_usage = measure_cpu_mem_during(compute_metrics_multiprocessing, df_pandas, symbols, lib="pandas", window=window)
    print(f"Multiprocessing (Pandas) - Time: {total_time:.2f}s, Avg CPU: {avg_cpu:.1f}%, Mem Usage: {mem_usage:.2f}%")

    # Threading (polars)
    threading_results_polars, total_time, avg_cpu, mem_usage = measure_cpu_mem_during(compute_metrics_threading, df_polars, symbols, lib="polars", window=window)
    print(f"Threading (Polars) - Time: {total_time:.2f}s, Avg CPU: {avg_cpu:.1f}%, Mem Usage: {mem_usage:.2f}%")

    # Multiprocessing (polars)
    multiprocessing_results_polars, total_time, avg_cpu, mem_usage = measure_cpu_mem_during(compute_metrics_multiprocessing, df_polars, symbols, lib="polars", window=window)
